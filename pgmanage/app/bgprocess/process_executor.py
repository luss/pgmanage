...
def _handle_execute_exception(ex, args, _stderr, exit_code=None):
    ...
    args.update({"exit_code": ex.errno if exit_code is None else exit_code})
...
def execute(argv):
    ...
    try:
        ...
        # Add end_time
        args.update({"end_time": datetime.now()})
    ...
    if _IS_WIN:
        ...
        if (
            ...
            if exitCode is not None:
                ...
                #     )
                # )
                print("[PARENT] Child exited with exit-code#{0}...".format(exitCode))
            else:
                # _log('[PARENT] Started the child with PID#{0}'.format(p.pid))
                print("[PARENT] Started the child with PID#{0}".format(p.pid))
        ...
        if os.fork() == 0:
            ...
            print("[CHILD] Forked the child process...")
            # Hmm... So - I need to do the job now...
            try:
                os.close(r)

                # _log('[CHILD] Make the child process leader...')
                print("[CHILD] Make the child process leader...")
                ...
                print("[CHILD] Inform parent about successful child forking...")
                w.write("1")
                w.close()

                # _log('[CHILD] Start executing the background process...')
                print("[CHILD] Start executing the background process...")
                execute(argv)
            except Exception:
                _log_exception()
                sys.exit(1)
            ...
            print("[PARENT] Got message from the child...")
            r.close()
