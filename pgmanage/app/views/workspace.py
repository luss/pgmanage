...
from app.utils.master_password import set_masterpass_check_text, validate_master_password, reset_master_pass
...
def index(request):
    ...
    template = loader.get_template('app/workspace.html')
...
def shortcuts(request):
    ...
    template = loader.get_template('app/shortcuts.html')
    return HttpResponse(template.render(context, request))
...
def master_password(request):
    ...
    if user_details.masterpass_check \
        and not validate_master_password(user_details, master_pass_hash):
        v_return['v_error'] = True
